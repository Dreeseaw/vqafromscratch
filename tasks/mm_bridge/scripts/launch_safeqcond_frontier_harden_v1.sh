...
if (( HORIZON_HOURS < 1 )); then
  HORIZON_HOURS=1
fi
if (( HORIZON_HOURS > 24 )); then
  HORIZON_HOURS=24
fi
...
TIMEOUT_SEC="$(( HORIZON_HOURS * 3600 ))"
...
echo "[safeqcond-frontier] HORIZON_HOURS=${HORIZON_HOURS} TIMEOUT_SEC=${TIMEOUT_SEC}"
...
if timeout --signal=INT --kill-after=30s "${TIMEOUT_SEC}s" "${CMD[@]}"; then
  exit 0
fi

status=$?
if [[ "${status}" -eq 124 || "${status}" -eq 137 ]]; then
  echo "[safeqcond-frontier] Horizon reached for ${RUN_ID}; restart with the same command to continue."
  exit 0
fi
