...
SWEEP_ID="ds_vmbench_v1_${STAMP}"
SWEEP_DIR="logs/${SWEEP_ID}"
mkdir -pv "${SWEEP_DIR}"
ln -sfn "${SWEEP_ID}" "logs/ds_vmbench_v1_latest"

VM_RUN_PREFIX="${VM_RUN_PREFIX:-vm_dinovit}"
MM_RUN_PREFIX="${MM_RUN_PREFIX:-mm_dinovit}"
DRY_RUN="${DRY_RUN:-0}"
...
# Datasetting VM DINO Benchmark V1

Sweep ID: ${SWEEP_ID}
Start time: $(date)

Two-stage experiment family structure:
1. VM SSL train via \`rundino.sh\`
2. Fixed downstream MM eval via \`runmm.sh\`

Run prefixes:
- VM_RUN_PREFIX=${VM_RUN_PREFIX}
- MM_RUN_PREFIX=${MM_RUN_PREFIX}

Runtime knobs:
- MM_TARGET_STEP=${MM_TARGET_STEP}
- VM batch=${VM_BATCH_SIZE}
- MM batch=${MM_BATCH_SIZE} grad_accum=${MM_GRAD_ACCUM}
EOF

printf "family\tvm_run_id\tmm_run_id\tvm_epoch_target\tvm_checkpoint\tdataset_mix_json\n" > "${SWEEP_DIR}/families.tsv"
...
run_vm_stage() {
  ...
  shift 3
  local run_id="${VM_RUN_PREFIX}_${family}"
  ...
  echo "[$(date)] START ${run_id} stage=vm family=${family} target_epoch=${epochs} resume_step=${resume_step}" | tee -a "${SWEEP_DIR}/timeline.log"
...
run_mm_stage() {
  ...
  shift 2
  local run_id="${MM_RUN_PREFIX}_${family}"
  ...
  echo "[$(date)] START ${run_id} stage=mm family=${family} target_step=${MM_TARGET_STEP} resume_step=${resume_step}" | tee -a "${SWEEP_DIR}/timeline.log"
  echo "[$(date)] CMD ${cmd[*]}" >> "${SWEEP_DIR}/timeline.log"
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[$(date)] DRY_RUN skip ${run_id}" | tee -a "${SWEEP_DIR}/timeline.log"
    return 0
...
run_family() {
  ...
  local vm_run_id="${VM_RUN_PREFIX}_${family}"
  local mm_run_id="${MM_RUN_PREFIX}_${family}"
  local vm_checkpoint="logs/${vm_run_id}/epoch_${vm_epochs}.tar"

  echo "[$(date)] FAMILY ${family} vm=${vm_run_id} mm=${mm_run_id}" | tee -a "${SWEEP_DIR}/timeline.log"
  if ! run_vm_stage "${family}" "${vm_epochs}" "${dataset_mix_json}" "$@"; then
    echo "[$(date)] FAIL  family=${family} stage=vm" | tee -a "${SWEEP_DIR}/timeline.log"
    return 1
  fi
  if [[ ! -f "${vm_checkpoint}" ]]; then
    echo "[$(date)] FAIL  family=${family} missing vm checkpoint ${vm_checkpoint}" | tee -a "${SWEEP_DIR}/timeline.log"
    return 1
  fi
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "${family}" "${vm_run_id}" "${mm_run_id}" "${vm_epochs}" "${vm_checkpoint}" "${dataset_mix_json}" >> "${SWEEP_DIR}/families.tsv"
  run_mm_stage "${family}" "${vm_checkpoint}"
}

run_family "v2" "100" "" || true
run_family "mixed1" "75" '{"coco_local:train2014":100,"textocr:train":100,"coco_text:train":100}' || true
