...
docker run --rm -it --gpus all -v "$(pwd)":/app -w /app myrepo:gpu \
	python -m train.train_transformer "$RUN_ID" \
		--train_data data/pretraining/wikicoco256_cleaned/train --val_data data/pretraining/wikicoco256_cleaned/val --test_data data/pretraining/wikicoco256_cleaned/test \
